...
LogAccess.prototype.execute = function (cxt, params) {
  ...
  var format = "\033[1;35m[%s]\033[1;36m===> %s\033[0m %s (%s) \033[1;32mIP: %s \033[1;34mUsername: %s\033[0m";
  ...
  if (headers.authorization) {
    var authStr = headers.authorization.match(/^Basic\s([a-zA-Z0-9=\+/]+)$/);
    if (authStr && authStr[1]) {
      var user = HTTPUtil.userFromAuthString(authStr[1]);
      username = user.name;
    }
  }

  cxt.log(format, date, request.method, request.url, host, request.connection.remoteAddress, username, level);
