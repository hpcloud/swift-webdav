...
HandleLock.prototype.createLock = function () {
  ...
  if (!this.resource) {
    var file = this.bridge.createResource(this.path);
    var empty = new require('pronto/streams').StringReader('');
...
HandleLock.prototype.parseTimeout = function(timeout) {
  var tStr = timeout || this.req.header.timeout;
  var seconds = 0;
  if (!tStr) {
    return seconds;
  }
...
HandleLock.prototype.cannotLock = function () {
  ...
  if (lock.isExclusive() && this.token != lock.token) {
    return true;
  }
