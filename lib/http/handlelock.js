...
HandleLock.prototype.execute = function (cxt, params) {
  ...
  // An existing lock
  // FIXME: need support for shared locks.
  this.lock = this.resource ? this.resource.lock(0) : null;//params.lock;
  ...
  this.parent = params.parent ? params.parent.lock(0) : null;
...
HandleLock.prototype.refreshLock = function () {
  ...
  if (!this.lock || this.lock.token != this.token) {
    // If no lock, but there is a parent lock, do an "indirect refresh."
    if (this.parent && this.parent.token == this.token) {
  ...
  var self = this;
  this.bridge.lock(this.resource, this.lock, function (e) {
    if (e) {
      if (e.status) {
...
HandleLock.prototype.buildLock = function () {
  ...
  this.bridge.lock(this.resource, lock, function (e) {
    // Lock is not issued.
    if (e) {
      if (e.status) {
        self.reroute('@' + e.status, self.cxt);
