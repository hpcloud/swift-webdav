...
HandleProppatch.prototype.handleUpdates = function (updatesNL) {
  ...
  // Process the queue.
  this.mergeProperties(resource, queue);

  bridge.saveProperties(resource, function (e, multistatus) {
    if (e) {
      cmd.cxt.log("A bad thing happened to our properties: ", e.message, "error");
    }
...
HandleProppatch.prototype.setProperties = function (resource, properties) {
  var oldProps = reqource.properties();
  var found = false;
  for (var i = 0; i < properties.length; ++i) {
  }
}
