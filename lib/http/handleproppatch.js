...
HandleProppatch.prototype.handleUpdates = function (updatesNL) {
  ...
  for (var i = 0; i < updatesNL.length; ++i) {
    ...
    var isInvalid = this.validateProperties(properties, cmd);
  ...
  if (isInvalid) {
    ...
    // If this is an Error object, it's a fatal error.
    if (isInvalid instanceof Error) {
      this.reroute('@' + isInvalid.status, this.cxt);
      return;
    }

    // This generates a 207 - MultiStatus.
...
HandleProppatch.prototype.validateProperties = function (properties, verb) {
  ...
  for (var i = 0; i < properties.length; ++i) {
    ...
    if (!p.ns || p.ns.length == 0) {
      p.status = 400;
      validationError = new Error('Malformed XML');
...
HandleProppatch.prototype.mergeProperties = function (resource, queue) {
  ...
  for (var i = 0; i < queue.length; ++i) {
    ...
    modifiedProps = modifiedProps.concat(queue[i].props);
  }
...
HandleProppatch.prototype.removeProperties = function (resource, properties) {
  ...
  for (var j = 0; j < oldProps.length; ++j) {
    var cname = oldProps[j].clarkName()
    if (toDelete.indexOf(cname) >= 0) {
      this.cxt.log("Removing property %s", cname, "debug");
    }
    else {
