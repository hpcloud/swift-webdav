...
HandlePropfind.prototype.fileProperties = function (file, property) {
  ...
  var seekName = property.ns == 'DAV:' ? property.name : XMLUtil.clarkNS(property.ns, property.name);
  ...
  switch (seekName) {
    ...
    default:
      ...
      this.searchPropertyStorage(property, file.properties());
  }
}
...
HandlePropfind.prototype.collectionProperties = function (collection, property) {
  ...
  switch (property.name) {
    ...
    default:
      // Not very happy with this.
      this.searchPropertyStorage(property, collection.properties());
  }
}
...
HandlePropfind.prototype.searchPropertyStorage = function(property, storedProperties) {
  ...
  if (storedProperties) {
    for (var i = 0; i < storedProperties.length; ++i) {
      ...
      if(XMLUtil.clarkNS(p.ns, p.name) == seekName) {
