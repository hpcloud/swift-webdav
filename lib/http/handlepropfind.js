...
HandlePropfind.prototype.execute = function (cxt, params) {
  ...
  if (!this.resource || !this.resource.isSamePathAs(this.path)) {
    var rname = this.resource ? this.resource.name(): "(NO SUCH RESOURCE)";
    cxt.log("Resource not found for propfind: %s, %s", this.path, rname, "debug");
 ...
 */
HandlePropfind.prototype.handleProps = function (noVals, no404s) {
  ...
  var rname = this.resource.name();

  // Reports suggest that some clients expect trailing slashes on
  // collections.
  if (this.resource.isCollection && rname != '/') {
    rname += '/';
  }
  multistatus.addProperties(rname, props);
  ...
  if (this.depth == 0) {
    ...
    this.done(multistatus.toXML(noVals, no404s));
  }
  // Get descendant props for the collection.
    ...
    this.bridge.listContents(this.resource, deep, function (e, resources) {
      ...
      self.done(multistatus.toXML(noVals, no404s));
    });
  }
