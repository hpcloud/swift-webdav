...
Util.isoDate = function (time) {
  ...
  //var fmt = '%Y-%m-%dT%R%Z';
  //return (new Date(time)).toLocaleFormat(fmt);
  return (new Date(time)).toISOString(); //.replace(/\.[0-9]{3}Z/, 'Z');
}
...
Util.checkPreconditions = function (request, resource) {
  ...
  var modTime = resExists ? Date.parse(resource.lastModified()) : Date.now();
  ...
  // in the request."
    ...
    // console.log("@@@@@ Comparing if-modified-since %s with modification time %s", lastTime, modTime);
 ...
 *   The HTTP the authorization header.
 *
 * @return {Object}
 *   An Object with two properties of name and pass. If the string is invalid
 *   nothing is returned.
 */
Util.userFromAuthString = function (authorization) {

  var authString = authorization.match(/^Basic\s([a-zA-Z0-9=\+/]+)$/);
  if (!authString || !authString[1]) {
    this.cxt.log("Authentication type is not Basic.", "debug");
    return;
  }

  var buff = new Buffer(authString[1], 'base64');
