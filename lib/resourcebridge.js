...
ResourceBridge.prototype.setProperty = function (resource, property, fn) {
  ...
  var e = new Error('Cannot store properties.');
  e.status = 500;
  fn(e);
}
/**
...
ResourceBridge.prototype.removeProperty = function (resource, property, fn) {
  ...
  var e = new Error('Cannot remove properties.');
  e.status = 500;
  fn(e);
}

/**
 * Load properties into a resource.
 *
 * For the most part, this is considered an internal function, though
 * there is no reason it cannot be called externally.
 */
ResourceBridge.prototype.loadProperties = function (resource, propertyStore, fn) {
  propertyStore.get(resource.name(), function (e, properties) {
