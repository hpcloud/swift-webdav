...
FSBridge.prototype.load = function (resourceName, fn) {
  ...
  fs.stat(name, function (e, stat) {
    ...
    if (stat.isFile()) {
      ...
      self.loadProperties(file, self.propertyStore, function () {
        fn(false, file);
      });
    }
      ...
      // fn(false, col);
      self.loadProperties(col, self.propertyStore, function () {
        fn(false, col);
      });
    }
    else {
...
FSBridge.prototype._loadResources = function (paths, resources, done) {
  ...
  fs.stat(head, function (e, stats) {
    ...
    self.loadProperties(res, self.propertyStore, function () {
      // Recurse.
      self._loadResources(paths, resources, done);
    });
  });
