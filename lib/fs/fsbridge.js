 ...
 * KNOWN ISSUES:
 *
 * - Currently does not recursively copy dead properties for
 *   collections.
 * - Does not recursively delete properties for collections.
 */
...
function FSBridge(documentRoot, baseURI, cxt, propertyStore) {
...
FSBridge.prototype.load = function (resourceName, fn) {
  ...
  fs.stat(name, function (e, stat) {
    ...
    if (stat.isFile()) {
      ...
      self.loadProperties(file, function () {
        fn(false, file);
      });
    }
      ...
      // fn(false, col);
      self.loadProperties(col, function () {
        fn(false, col);
      });
    }
    else {
...
FSBridge.prototype.save = function (resource, stream, fn) {
  ...
  stream.on('end', function () {
    ...
    // 5. Save properties (Do we really want to do this? Side effects?)
    self.propertyStore.set(resource.name(), resource.properties(), function() {
      // 6. Return save data.
...
FSBridge.prototype.deleteCollection = function (collection, fn) {
  ...
  FSBridge.rrmdir(path, function (e, failures) {
    ...
    for (var i = 0; i < failures.length; ++i) {
      failures[i].href = cmd.uriTo(failures[i].path);
    }
    ...
    cmd.propertyStore.remove(collection.name(), function () {
      fn(false, failures);
    });
...
FSBridge.prototype.deleteFile = function (resource, fn) {
  ...
  fs.unlink(path, function (e) {
    ...
    self.propertyStore.remove(resource.name, function () {
...
FSBridge.prototype.copy = function (src, dest, overwrite, fn) {
  ...
  _overwriteCheck(function (srcPath, destPath, fn) {
    ...
    FSBridge.copyFile(srcPath, destPath, function (e, data) {
      // Extra hoop jump to save properties.
      self.propertyStore.set(dest, properties, function () {
        fn(e, data);
      });
    });
  });
...
FSBridge.prototype.loadProperties = function (resource, fn) {
  this.propertyStore.get(resource.name(), function (e, properties) {
    resource.setProperties(properties);
    fn(resource);
  });
}

FSBridge.prototype.saveProperties = function (resource, fn) {
  this.propertyStore.set(resource.name(), resource.properties(), function (e) {
    if (e) {
      e.status = 500;
    }
    fn(e, resource);
...
FSBridge.prototype._loadResources = function (paths, resources, done) {
  ...
  fs.stat(head, function (e, stats) {
    ...
    // Load the properties for this source.
    self.loadProperties(res, function () {
      // Recurse.
      self._loadResources(paths, resources, done);
    });
  });
...
FSBridge.rcopydir = function (src, dest, fn) {
  fs.readdir(src, function (e, contents) {
    ...
    _listAllFiles(newPaths, [src], true, function (e, srcs) {
        if (e) {
          fn(e);
          //console.log(e);
          return;
...
FSBridge.rrmdir = function (dir, fn) {
  fs.readdir(dir, function (e, contents) {
    ...
    _listAllFiles(newPaths, [dir], true, function (e, toDelete) {
        if (e) {
          fn(e);
          //console.log(e);
          return;
  ...
  fs.stat(head, function (e, stat) {
    ...
    if (shallow && stat.isDirectory()) {
      ...
      fs.readdir(head, function (e, contents) {
        ...
        _listAllFiles(newPaths.concat(_in), _out, shallow, done);
      });
    }
    else {
      // console.log('File %s', head);
      _listAllFiles(_in, _out, shallow, done);
    }
  });
}
