  ...
  if (memcached) {
    ...
    memcached.get(cacheId, function(err, result) {
      if (err) {
        ...
        cxt.log("\033[1;35m[%s]\033[0m Error retrieving %s from memcached. Message: %s", date, cacheId, err);
      }
          ...
          if (expireIn > 300) {
            memcached.set(cacheId, data, expireIn, function(err, result) {
              if (err) {
                // In case of an error we log it. Log all the things!
                var date = new Date().toUTCString();
                cxt.log("\033[1;35m[%s]\033[0m Error storing %s in memcached. Message: %s", date, cacheId, err);
              }
