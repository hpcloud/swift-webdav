...
SwiftBridge.prototype.deleteDirectory = function (resource, path, fn) {
  var self = this;
  this.cxt.log("Get the container: deleteDirectory", "custom");
  this.store.container(this.containerName, function (e, container) {
    if (e) {
      e.status = 404;
      fn(e);
    ...
    // object.
    self.cxt.log("get all objects: deleteDirectory", "custom");
...
SwiftBridge.prototype._deleteFile = function (file, path, container, fn) {
  this.cxt.log("Delete file: deleteFile", 'custom');
...
SwiftBridge.prototype.copy = function (src, dest, overwrite, fn){
  ...
  function _overwriteCheck(container, goAheadAndCopy) {
    // If it's okay to overwrite, ignore all this crap.
    if (overwrite) {
      goAheadAndCopy();
    }

    // Check #1: If the containers are different, we need to
    // see if...
    // a) The destination container exists, and if it does...
    // b) does the destination path also exist?
    else if (container.name() != destParts.containerName) {
      // Load the destination container.
      self.loadContainer(destParts.containerName, function (e, destContainer) {
        // If the destination container does not exist, we're good.
        if (e) {
          goAheadAndCopy();
          return;
        }

        // If we get here, we need to see if the destination exists.
        self.loadObject(destParts.path, dest, destContainer, function (e, oInfo) {
          // Not overwriting... go ahead.
          if (e) {
            goAheadAndCopy();
            return;
          }
          // Uh-oh, something's already there. Bail out.
          var err = new Error('Cannot overwrite container.');
          err.status = 412;
          fn(err);
          return;
        });
    ...
    // Check #2: If the containers are the same, we just need to make
    // sure the destination does not already exist.
    else {
      self.loadObject(destParts.path, dest, container, function (e, oInfo) {
        // Nothing at that location. Go ahead.
        if (e) {
          goAheadAndCopy();
          return;
        }
        // Uh-oh. Something there already.
        var err = new Error('Cannot overwrite container.');
        err.status = 412;
        fn(err);
        return;
  ...
  */
  ...
  // Dear RFC-4918,
  //
  // In your thoughtful, but ultimately frustrating, commentary in
  // section 9.8.4, you state that:
  //
  ...
  //
  // While in some possible universe this behavior may make sense,
  // Windows, OSX, and UNIX simply don't work this way. So am I supposed
  // to break the expectations of every user on the planet to satisfy
  // your whim? I think not. As Cicero, or was it the Spice Girls,
  // once so eloquently put it, "If you wanna be my luva, you gotta hang
  // with my friends." I don't want my friends to lose their data over
  // my relationship with you, so I think we'd all be better off if you
  // and I were to, well, maybe not have such a close relationship.
  //
  ...
  // Is this container to container, or directory to directory?
  this.store.container(this.containerName, function (e, container) {
    if (e) {
      e.status = 404;
      fn(e);
    ...
    var copyFrom = self.resourceToObject(src);
    self.cxt.log('Copying %s to %s in %s', copyFrom.name(), destParts.path, destParts.containerName, 'debug');
    // Only fire the function if we pass the overwrite rules.
   // _overwriteCheck(container, function() {
      container.copy(copyFrom, destParts.path, destParts.containerName, function (e) {
        if (e) {
          e.status = e.statusCode == 404 ? 404 : 403;
        }
        fn(e);
      });
    });
