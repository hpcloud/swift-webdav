...
SwiftBridge.prototype.load = function (name, skipBody, fn){

  // Compat with original API.
  if (typeof skipBody == 'function') {
    this.cxt.log("Legacy call to SwiftBridge::load", "info");
    fn = skipBody;
    skipBody = false;
  }

  var now = Date.now();
  // this.cxt.log("SwiftBridge load %s", name, "debug");
  if (!this.identity.tenantId()) {
    ...
    this.cxt.log("Container root", "debug");
    // Tenant root.
    ...
    var self = this;
    this._ccache(this.containerName, function (e, container){
      if (e) {
        e.status = 404;
        fn(e);
        return;
      ...
      if (path) {
        ...
        // If skipBody is set, we try to HEAD instead of GET. This can
        // be a huge performance boon.
        if (skipBody) {
          self.loadObjectInfo(path, name, container, fn);
        }
        else {
  ...
  if (path.length == 0) {
    ...
    this.deleteDirectory(resource, path, function (e, multi) {
      ...
      self.store.deleteContainer(self.containerName, function (e, wasThere) {
        self._time("deleteCollection", self.containerName, stime);
        // Delete this item out of the container cache.
        delete self._containerCache[self.containerName];
...
SwiftBridge.prototype._deleteFile = function (file, path, container, fn) {
  ...
  container.delete(path, function (e, status) {
    self._time("_deleteFile", file.name(), stime);
    if (e) {
      // Real error. 403?
      e.status = 403;
      fn(e, file.name());
...
SwiftBridge.prototype.copy = function (src, dest, overwrite, fn){
  ...
  function _eachItem(item, srcContainer, cb) {
    ...
    // Run the copy.
    self.cxt.log("Copying individual item from %s to %s in %s: copy._eachItem", 
                 item.name(), destPath, destParts.containerName, "debug");
    var stime = Date.now();
    srcContainer.copy(item, destPath, destParts.containerName, function (e) {
      self._time("copy._eachItem", destPath, stime);
      if (e) {
  ...
  function _done(e, container) {
    ...
    self.cxt.log("Copying %s to %s in %s: copy._done", o.name(), destParts.path, destParts.containerName, "debug");
  ...
  // an existing container IFF that container exists already.
    ...
    this._ccache(destParts.containerName, function (e, container) {
  ...
  // exists. Annoyingly, this adds another REST request.
    ...
    destContainer.objectsWithPrefix(parent, '', function (e, list) {
      self._time("copy", "parent " + parent, pstime);
      //console.log(list);
      if (e) {
  ...
  if (path) {
    this._ccache(this.containerName, function (e, container) {
      ...
      container.updateObjectMetadata(info, function (e) {
        self._time("saveProperties", path, stime);
    ...
    var stime2 = Date.naow();
    this.store.createContainer(name, null, info.metadata(), function (e, data) {
      self._time("saveProperties", 'Container ' + name, stime2);
      fn(e);
    });
  }
}
...
SwiftBridge.prototype.resourceToObject = function (resource) {
  ...
  if (lock) {
    ...
    metadata['exclusive-lock'] = SwiftUtils.serializeLock(lock);
  }
...
SwiftBridge.prototype._time = function(label, detail, start, end) {
  if (end == undefined) {
    end = Date.now();
  }
  this.cxt.log("%d ms: %s (%s)", end - start, label, detail, "custom");

}

/**
 * Load object info, and correct for directory markers.
 *
 ...
 */
SwiftBridge.prototype.loadObjectInfo = function (path, name, container, fn) {
  var self = this;
  var stime = Date.now();
  container.objectInfo(path, function (e, obj) {
    self._time("loadObjectInfo", name, stime);
    if (e) {
    ...
    if (self.isDirectoryMarker(item)) {
      ...
      // XXX: Should we have an empty reader here?
...
SwiftBridge.prototype.loadObject = function (path, name, container, fn) {
  ...
  container.remoteObject(path, function (e, obj) {
    self._time("loadObject", name, stime);
    if (e) {
...
SwiftBridge.prototype._loadLock = function (resource, info) {
  ...
  for (var md in metadata) {
    if (md == 'exclusive-lock') {
      this.cxt.log("Lock: %s", metadata[md], "debug");
      // var rawlock = SwiftUtils.decodeHeaderValue(metadata[md], true);
      var rawlock = SwiftUtils.unserializeLock(metadata[md], resource);
      if (rawlock) {
        ...
        console.log("Lost lock %s", metadata[md], "error");
      }
      break;
    }
  }
...
SwiftBridge.prototype.loadSubdir = function (path, name, container, fn) {
  var self = this;
  this.cxt.log("Check if subdir exists: loadSubdir.", "debug");
...
SwiftBridge.prototype.listProjects = function (fn) {
  ...
  is.tenants(this.identity, function (e, projects) {
    self._time("listProjects", 'all', stime);
    if (e) {
      fn(e);
      return;
    }
...
SwiftBridge.prototype.pathTo = function (resourceName, trailingSlash, baseURI) {
  ...
  // For logging.
  var oResourceName = resourceName;

  var tname = resourceName + '/'; // force boundary.
  if (tname.indexOf(this.baseURI) == 0) {
    var len = baseURI.length;
    resourceName = resourceName.substring(len);
  }

  if (!slash && resourceName.slice(-1) == '/') {
    this.cxt.log("Removing trailing slash.", "debug");
    resourceName = resourceName.slice(0, -1);
  }

  this.cxt.log("Adjusting %s relative to %s yields %s", oResourceName, baseURI, resourceName, "debug");
...
SwiftBridge.prototype.listContainers = function (fn) {
  ...
  this.store.containers(function (e, list) {
    self._time("listContainers", 'all', stime);
    if (e) {
...
SwiftBridge.prototype.listObjects = function (collection, deep, fn) {
  ...
  this.loadContainer(this.containerName, function (e, container) {
    ...
    container.objectsWithPrefix(path, delimiter, function (e, list) {
      self._time("listObjects", path, stime);
...
SwiftBridge.prototype.mkContainer = function (collection, fn) {
  ...
  this._ccache(cname, function (noContainer, container) {
    if (noContainer) {
...
SwiftBridge.prototype.mkDirectory = function (collection, fn) {
  ...
  this._ccache(this.containerName, function (noContainer, container) {
    ...
    if (parent != '/' && parent != '.') {
      ...
      container.objectsWithPrefix(parent, function (e, list) {
        self._time("mkDirectory", parent, stime);
        ...
        container.save(marker, '', function (e, r) {
          self._time("loadObjectInfo", "save " + marker, stime2);
          if (e) {
            e.status = 403;
          }
          fn(e);
      ...
      var stime3 = Date.now();
      container.save(marker, '', function (e, r) {
        self._time("loadObjectInfo", "save " + marker, stime);
        if (e) {
          e.status = 403;
        }
        fn(e);
...
SwiftBridge.prototype.withEachObjectIn = function (path, containerName, eachItem, done) {
  var self = this;
  this._ccache(containerName, function (e, container) {
    if (e) {
      e.status = 404;
      done(e);
      return;
    ...
    var stime = Date.now();
    container.objectsWithPrefix(path + '/', function (e, list) {
      self._time("withEachObjectIn", path, stime);
      if (e) {
 ...
 * Container cache.
 *
 * This may be highly volatile.
 */
SwiftBridge.prototype._ccache = function (containerName, fn) {
  var self = this;
  if (this._containerCache[containerName]) {
    this.cxt.log("Using cached container %s", containerName, "info");
    fn(false, this._containerCache[containerName]);
    return;
  }
  var stime = Date.now();
  this.store.container(containerName, function (e, container){
    self._time("_ccache", containerName, stime);
    if (!e) {
      self._containerCache[containerName] = container;
    }
    fn(e, container);
  });
}
