...
SwiftBridge.prototype.copy = function (src, dest, overwrite, fn){
  ...
  // an existing container IFF that container exists already.
  else if (srcPath.length == 0 && this.containerName != destParts.containerName) {
      ...
      if (e) {
        ...
        if (e.statusCode == 404) {
          ...
          self.store.createContainer(destParts.containerName, acl, {}, function (e, data) {
            ...
            self.withEachObjectIn(srcPath, self.containerName, _eachItem, fn);
          });
        }
    ...
    /*
    this.cxt.log("Cannot copy a container to another container. %s", srcPath, "warning");
    var err = new Error('Copying containers is not allowed.');
    err.status = 403;
    fn(err);
    return;
   */
