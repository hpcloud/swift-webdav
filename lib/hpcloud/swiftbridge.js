...
SwiftBridge.prototype.loadLock = function (resource, info) {
  var metadata = info.metadata();

  // Find locks:
  for (var md in metadata) {
    if (md == 'exclusive-lock') {
      var rawlock = SwiftUtils.decodeHeaderValue(metadata[md]);
      if (rawlock) {
        this.cxt.log("Found lock: %j", rawlock, "debug");
