...
setupMemcached.prototype.execute = function (cxt, params) {
  ...
  if (!params.settings.servers) {
	...
	cache.on('issue', cmd.eventOnIssue);
	cache.on('failure', cmd.eventOnFailure);
	cache.on('reconnecting', cmd.eventOnReconnecting);
	cache.on('reconnected', cmd.eventOnReconnected);
	cache.on('remove', cmd.eventOnRemove);
...
setupMemcached.prototype.eventOnIssue = function (issue) {
	...
	cmd.cxt.log(format, date, issue.server, issue.retries, issue.messages.join( '' ), 'warning');
}
...
setupMemcached.prototype.eventOnFailure = function (issue) {
	...
	cmd.cxt.log(format, date, issue.server, issue.messages.join( '' ), 'error');
}
...
setupMemcached.prototype.eventOnReconnecting = function (issue) {
	...
	cmd.cxt.log(format, date, issue.server, issue.messages.join( '' ), 'info');
}
